...
const ResidencyVariants: FC<ResidencyVariantsProps> = ({
  icons,
  variants,
  value,
}) => {
  ...
  return (
    <VariantsContainer>
      <VariantsContent>
        <VariantsTitle>Unidades</VariantsTitle>
        ...
        <VariantsUnit active={true}>
          ...
          <UnitInfo>
            <UnitType>{suite ? "Suíte" : "Área Privativa"}</UnitType>
            <UnitPrice>R$ {value}</UnitPrice>
          </UnitInfo>
          <SelectedUnit>
            <p>Você está vendo esta unidade!</p>
            <div />
          </SelectedUnit>
        </VariantsUnit>

        <DiferentUnit active={existVariants}>
          <VariantsSubTitle>Outras Unidades</VariantsSubTitle>

          {variants.map(
            (
              { value, suite, firstCharacteristic, secondCharacteristic }: any,
              index
            ) => {
              return (
                <VariantsUnit key={index} active={false}>
                  <UnitIcons>
                    <UnitFigure>
                      <img
                        src={`/icons/${firstCharacteristic?.icon}`}
                        alt={firstCharacteristic?.name}
                      />
                      <figcaption>{firstCharacteristic?.name}</figcaption>
                    </UnitFigure>
                    <UnitFigure>
                      <img
                        src={`/icons/${secondCharacteristic?.icon}`}
                        alt={secondCharacteristic?.name}
                      />
                  ...
                  <NotSelectedUnit>
                    <p>Você poderá ver esta unidade</p>
                    <button>
                      <Eye />
                      Ver Unidade
                    </button>
